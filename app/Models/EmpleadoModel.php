<?php
 ...
 class EmpleadosModel
 {
	...
	public function dameEmpleados_monitor_ad() 
	{
		$sql = "SELECT e . * , d.nombre departamento_name, p.nombre puesto_name, r.nombre Empresa, p.concepto consepto, p.cuenta_contable cta_contable, p.centro_costos centro_costos, (prh.saldo / prh.plazos_max) deuda  FROM empleados e, departamentos d, puestos p, razones_sociales r , prestamos_rh prh WHERE e.departamento_id = d.departamento_id AND e.puesto = p.clave and d.area_id= '2' and e.razon_social_id=r.razon_social_id and prh.empleado_id=e.empleado_id  order by d.orden, p.orden";
		$result = mysql_query($sql, $this->conexion);
		$empleados = array();
		if($result){
			while ($row = mysql_fetch_assoc($result))
	...
	public function dameEmpleados_monitor_ad_del_historial() 
	{
		$sql = "SELECT  * FROM nom_nomias_historico WHERE semana = YEARWEEK(NOW())";
		$result = mysql_query($sql, $this->conexion);
		$empleados = array();
		if($result){
			while ($row = mysql_fetch_assoc($result))
	...
	public function dameEmpleados_monitor_ad_id() // ajustar a postgress
	{
		$sql = "SELECT e .empleado_id  FROM empleados e, departamentos d, puestos p, razones_sociales r WHERE e.departamento_id = d.departamento_id AND e.puesto = p.clave and d.area_id= '2' and e.razon_social_id=r.razon_social_id  order by d.orden, p.orden";
